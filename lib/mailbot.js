...
class MailBot {
  ...
  async searchMessages (searchCriteria) {
    // With no search criteria it will fetch everything
    if(!searchCriteria) {
      searchCriteria = ['ALL']
    }
    const fetchOptions = { bodies: [ 'HEADER', 'TEXT' ], struct: true }

    //Converts non US_ASCII characters
    searchCriteria = JSON.parse(JSON.stringify(searchCriteria).normalize("NFD").replace(/\p{Diacritic}/gu, ""))

    const searchResult = await this.connection.search(searchCriteria, fetchOptions)
  ...
  searchAttachments (message, config) {
    const { allowed } = this.config.attachments
    const attachments = []
  ...
  async moveMessage (message) {
    if (this.config.moveProcessedMessages !== true) {
      console.log(`Move messages disabled by config`)
      return
    }
  ...
  async deleteMessage (message) {
    ...
    this.connection.deleteMessage(messageUid)
  }
