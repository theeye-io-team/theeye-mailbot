...
module.exports = {
   ...
   * @return {DateTime} luxon datetime instance
   */
  ...
  /**
    ...
    */
  buildCacheTimestampName (date, {startOfDay, timezone}) {
    const currentDate = date ?
      DateTime.fromISO(date).setZone(timezone) :
      DateTime.now().setZone(timezone) ;

    const startDate = this.timeExpressionToDateLuxon(
      startOfDay,
      timezone,
  ...
  buildRuntimeDate (date, { startOfDay, timezone }) {
    ...
    const runtimeDate = this.timeExpressionToDateLuxon(
      startOfDay,
      timezone,
      currentDate.toJSDate()
    )
