...
class API {
  ...
  checkExists (payload) {
    ...
    const options = {
      url:`${this.config.protocol}://${this.config.hostname}/api/Mails/check?${query}&access_token=${this.config.accessToken}`,
      method: 'GET',
      headers: {
  ...
  upload (payload, content) {
    ...
    const options = {
      url:`${this.config.protocol}://${this.config.hostname}/api/Mails/upload?access_token=${this.config.accessToken}`,
      method: 'POST',
      formData
    }
